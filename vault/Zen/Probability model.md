...
Formally, a probability model is a [Measure space](Measure%20space.md) $(\xi, \mathcal{F}, \mathbb{P})$ with the additional requirement $\mathbb{P}(\xi) = 1$,
i.e. at least one event must occur.
As an overview,

* $\xi$ represents the set of mutually exclusive outcomes (world-states);
* $\mathcal{F} \sube 2^\xi$ is a [σ-algebra](%CF%83-algebra.md) of possible events closed under compliment, finite union, and finite intersection; and
...
The [σ-algebra](%CF%83-algebra.md) contains at least $\xi$ and $\0$,
...
Some of these follow from measure space [Measure space > Properties](Measure%20space.md#properties)

1. $\mathbb{P}(\0) = 0$
1. $\mathbb{P}$ is monotone on $\mathcal{F}$ ordered by inclusion, i.e. $A \sube B \implies \mathbb{P}(A) \leq \mathbb{P}(B)$.
1. For any $A \in E$, it holds that $\mathbb{P}(A^c) = 1 - P(A)$
