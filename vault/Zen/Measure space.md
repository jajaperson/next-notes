...
A **measurable space** $(X, \Sigma)$ consists of a set $X$ and a [σ-algebra](%CF%83-algebra.md) on that set $\sigma$.
...
1. **empty set** has zero measure[^sat]: $\mu(\0) = 0$ ^M2
1. **σ-additivity**: $\mu(A \cup B)= \mu(A) + \mu(B)$ for any $A,B \in \Sigma$ with $A \cap B = \0$.
...
Thus a measure space generalises *volume* in the same way that a [metric space](Metric%20space.md) generalises length.
...
1. **countable subadditivity**: Let $\{E_i\}_{i=1}^\infty \sube \Sigma$ be a countable (or finite[^trailing]) sequence of measurable sets, then
   $$
  \begin{align*}
...
[^trailing]: Just give the sequence trailing $\0$.
 ...
 > Then by [σ-algebra > ^M5](%CF%83-algebra.md#m5) $A \setminus B \in \Sigma$ and $A = B \cup (A \setminus B)$,
 > so by [^M3](Measure%20space.md#m3) $\mu(A)= \mu(B) + \mu(A \setminus B)$, wherefore $\mu(A) \leq \mu(B)$, proving [^P1](Measure%20space.md#p1).
 > 
...
\end{align*}
$$
 > and by [^P1](Measure%20space.md#p1)
 > $$
\begin{align*}
...
\end{align*}
$$
 > applying this argument inductively proves [^P2](Measure%20space.md#p2).
 > <span class="QED"/>
